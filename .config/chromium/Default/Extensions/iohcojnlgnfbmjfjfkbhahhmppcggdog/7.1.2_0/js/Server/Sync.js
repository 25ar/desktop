(function(){
  ...
  if (window == chrome.extension.getBackgroundPage()) {
    ...
    var Sync = function(){
      ...
      const SESSION_COOKIE_NAME = "eversessionid";

      if( IS_LOCAL ){
        var SYNC_ADMIN_URL = "http://globalhost/sync_client";
      }
      else if( IS_TESTING ){
        var SYNC_ADMIN_URL = "http://testadmin.everhelper.me";
      }
      else{
      ...
      this.acquireSyncLock = function( callback ){
        ...
        console.error("Sync lock emulation");
        return callback(0);
