(function() {
	...
	HistoryComplete.prototype = {
		get _cache() {
			var c = localStorage["historycomplete:history_cache"];
		...
		set _cache(v) {
			localStorage["historycomplete:history_cache"] = JSON.stringify(v);
		},
			...
			if (this._cache == null) {
				chrome.history.search({
					...
					for (var i = 0; i != _results.length && results.length < that._maxCacheRecords; i++) {
						...
						if (urllower.indexOf("chrome") == 0 || urllower.indexOf("file:///") === 0 || urllower.indexOf("data:") === 0 ) {
							continue;
						}
						...
						var host = "";
						try{
							host = fvdSpeedDial.Utils.parseUrl( urllower, "host" ).replace(/^www\./, "");
						}
						catch( ex ){
